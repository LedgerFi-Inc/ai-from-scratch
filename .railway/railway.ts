...
export default defineRailway((ctx) => {
  ...
  const common = { source: repo, replicas: 1 };
    ...
    // Railway injects PORT=8080 into these services. Internal DNS entries are
    // hostnames only, so consumers must include the scheme and port explicitly.
    const api = service("api", { ...common, root: "app_ai_from_scratch", build: { builder: "DOCKERFILE", dockerfilePath: "api/Dockerfile", watchPatterns: ["api/**", "auth/**", "package.json", "pnpm-lock.yaml"] }, preDeploy: "pnpm db:deploy", healthcheck: "/api/health", env: { DATABASE_URL: db.env.DATABASE_URL, REDIS_URL: cache.env.REDIS_URL, DATA_URL: "http://data.railway.internal:8080", JWT_SECRET: { preserveExisting: true }, DATA_SECRETO: { preserveExisting: true }, AI_SECRETO: { preserveExisting: true }, QUEUE_SECRETO: { preserveExisting: true }, WEB_ORIGIN: prod ? "https://aifromscratch.shop" : "https://web-dev-a8ad.up.railway.app", NODE_ENV: "production", APP_ENV: prod ? "PROD" : "DEV" } });
    const web = service("web", { ...common, root: "app_ai_from_scratch/web", build: { builder: "DOCKERFILE", dockerfilePath: "Dockerfile", watchPatterns: ["**"] }, env: { API_URL: "http://api.railway.internal:8080", APP_ENV: prod ? "PROD" : "DEV" }, domains: prod ? ["aifromscratch.shop"] : [], healthcheck: "/healthz" });
    const payments = service("payments", { ...common, root: "app_ai_from_scratch/payments", build: { builder: "DOCKERFILE", dockerfilePath: "Dockerfile", watchPatterns: ["**"] }, healthcheck: "/health", env: { DATABASE_URL: paymentsDb.env.DATABASE_URL, ENTITLEMENTS_URL: "http://api.railway.internal:8080", PAYMENTS_SECRET: { preserveExisting: true }, ENTITLEMENTS_SECRET: { preserveExisting: true }, NODE_ENV: prod ? "production" : "development", APP_ENV: prod ? "PROD" : "DEV" } });
    const messages = service("messages", { ...common, root: "app_ai_from_scratch/messages", build: { builder: "DOCKERFILE", dockerfilePath: "Dockerfile", watchPatterns: ["**"] }, healthcheck: "/health", env: { DATABASE_URL: messagesDb.env.DATABASE_URL, MESSAGES_SECRET: { preserveExisting: true } } });
    const ai = service("ai", { ...common, root: "app_ai_from_scratch/ai", build: { builder: "DOCKERFILE", dockerfilePath: "Dockerfile", watchPatterns: ["**"] }, start: "sh -c '.venv/bin/uvicorn course_ai.app:app --host 0.0.0.0 --port ${PORT:-8799}'", healthcheck: "/salud", env: { NODE_URL: "http://api.railway.internal:8080", IA_SECRETO: { preserveExisting: true } } });
  ...
  const broker = service("rabbitmq", { source: image("rabbitmq:4-management-alpine"), replicas: 1, env: { AMQP_URL: { preserveExisting: true }, RABBITMQ_DEFAULT_PASS: { preserveExisting: true } } });
