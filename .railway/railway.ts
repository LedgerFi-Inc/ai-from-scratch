...
export default defineRailway((ctx) => {
  ...
  const common = { source: repo, replicas: 1 };
    ...
    const api = service("api", { ...common, root: "app_ai_from_scratch", build: { builder: "DOCKERFILE", dockerfilePath: "api/Dockerfile", watchPatterns: ["api/**", "auth/**", "package.json", "pnpm-lock.yaml"] }, preDeploy: "pnpm db:deploy", healthcheck: "/api/health", env: { DATABASE_URL: db.env.DATABASE_URL, REDIS_URL: cache.env.REDIS_URL, DATA_URL: "http://data.railway.internal:8080", JWT_SECRET: { preserveExisting: true }, DATA_SECRETO: { preserveExisting: true }, AI_SECRETO: { preserveExisting: true }, WEB_ORIGIN: prod ? "https://aifromscratch.shop" : "https://web-dev-a8ad.up.railway.app", NODE_ENV: "production", APP_ENV: prod ? "PROD" : "DEV" } });
    const web = service("web", { ...common, root: "app_ai_from_scratch/web", build: { builder: "DOCKERFILE", dockerfilePath: "Dockerfile", watchPatterns: ["**"] }, env: { API_URL: "http://api.railway.internal:8080", APP_ENV: prod ? "PROD" : "DEV" }, domains: prod ? ["aifromscratch.shop"] : [], healthcheck: "/robots.txt" });
